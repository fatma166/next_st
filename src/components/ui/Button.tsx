...
export const Button: FC<ButtonProps> = ({ children, onMouseEnter, isTextGradient, ...props }) => {
  ...
  const handleAnimate = async (y: number, r: number) => {
    ...
    setIsHovering(true);
    await animate([
      [".letters-down", { y: 0, rotate: r }, { duration: 0.6 }],
      [".letters", { y: -y, rotate: r }, { duration: 0.5, at: "<" }],
      [".btn-more", { x: 3, scale: 1.1 }, { duration: 0.5, at: "<" }],

      [".letters", { y: 0, rotate: 0 }, { duration: 0.000001, at: 0.6 }],
      [".letters-down", { y: y, rotate: 0 }, { duration: 0.000001, at: "<" }],
      [".btn-more", { x: 0, scale: 1 }, { duration: 0.3, at: "<" }],
    ]);
    setIsHovering(false);
  }
